...
1. **Grounded Conversational AI**:
   - Natural language queries (e.g., *"Will heavy rain affect Chennai tomorrow?"*, *"Should I irrigate my paddy field today?"*).
   - Zero hallucination with verified data provenance (IMD, GFS, ECMWF) and confidence ratings.
   - Explainable AI with "Why this advice?" scientific rationale.

2. **Multilingual Indian Language & Voice UI**:
   - Full support for **9 Indian languages**: English, தமிழ் (Tamil), हिन्दी (Hindi), తెలుగు (Telugu), മലയാളം (Malayalam), ಕನ್ನಡ (Kannada), বাংলা (Bengali), मराठी (Marathi), and ગુજરાતી (Gujarati).
   - Native Web Speech API Speech-to-Text (STT) and Text-to-Speech (TTS) audio broadcasts with real-time waveform visualizers.

3. **Interactive GIS Weather Map**:
...
8. **Demo Mode vs Live APIs**:
   - Out-of-the-box realistic synthetic dataset for Indian stations ensures 100% offline demonstration reliability.
...
│   │   │   ├── page.tsx          # Landing / Hero & Quick Prompts
│   │   │   ├── dashboard/        # Full Telemetry Dashboard
│   │   │   ├── assistant/        # Grounded AI Chat with Voice
...
│   │   ├── lib/                  # AI Engine, Translations, Mock Data, Weather API
